# -*- coding: utf-8 -*-
"""
Created on Fri Aug 28 02:53:51 2020
...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 21 01:05:37 2020
...
path = './FP4_1000/'
# 模型保存地址
model_path = './scenes_model12_1_4D100_1000_sigmoid.ckpt'
...
# name：操作的可选名称。
    ...
    with tf.variable_scope("layer5-conv3"):
        ...
        relu3 = tf.nn.sigmoid(tf.nn.bias_add(conv3, conv3_biases))
    ...
    with tf.variable_scope("layer7-conv4"):
        ...
        relu4 = tf.nn.sigmoid(tf.nn.bias_add(conv4, conv4_biases))
...
with tf.device('/gpu:0'):
    for epoch in range(n_epoch):
        ...
        for x_val_a, y_val_a in minibatches(x_val, y_val, batch_size, shuffle=False):
            err1, ac1 = sess.run([loss,acc], feed_dict={x: x_val_a, y_: y_val_a})
            val_loss += err1; val_acc += ac1; n_batch += 1
            
        print("   validation loss: %f" % (np.sum(val_loss)/ n_batch))
        print("   validation acc: %f" % (np.sum(val_acc)*3/ n_batch))
